...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post(`${baseUrl}/delegates/login`, {
        email,
        password,
      });
      setState({
        ...
        <div className="max-w-screen-xl m-0 sm:m-10 flex justify-center flex-1">
          <div className="lg:w-1/2 xl:w-5/12 p-6 sm:p-12">
            ...
            <div className="mt-5 flex flex-col items-center">
              <h1 className="text-xl xl:text-2xl font-bold">Login!</h1>
              <div className="w-full flex-1 mt-8">
                <form className="mx-auto max-w-xs" onSubmit={handleSubmit}>
                  ...
                  <a className="text-red-700 underline-offset-2" href="/forgot">Forgot Password?</a>
                </form>
              </div>
            </div>
          </div>
